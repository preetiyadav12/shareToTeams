...
const addUserToThread = (displayName: string, emoji: string, acsToken: any, token: any, acsID:string) => async (dispatch: Dispatch, getState: () => State) => {
  ...
  // create our user
  let userToken = await getToken(acsToken, token, acsID);
...
// Token Helper
const getToken = async (acsToken: any, token: any, acsID:string) => {
  try {
    var clientResponse = 
    '{ "user" : { "communicationUserId" : "'+acsID+'" }, "token" : "'+acsToken+'" }';
    return JSON.parse(clientResponse);
  } catch (error) {
